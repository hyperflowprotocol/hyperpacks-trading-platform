...
const usePresaleProgress = () => {
  const [totalRaised, setTotalRaised] = useState(593);
  const [progressPercentage, setProgressPercentage] = useState(59.3);
  const [realTimeBalance, setRealTimeBalance] = useState(64044000);
  ...
  const fetchProgress = useCallback(async () => {
    try {
      ...
      if (cached) {
        const cachedTotal = Math.max(parseFloat(cached), 593);

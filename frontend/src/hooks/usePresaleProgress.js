...
const usePresaleProgress = () => {
  const [totalRaised, setTotalRaised] = useState(502);
  const [progressPercentage, setProgressPercentage] = useState(16.73);
  ...
  const fetchProgress = useCallback(async () => {
    try {
      ...
      if (cached) {
        const cachedTotal = Math.max(parseFloat(cached), 502);

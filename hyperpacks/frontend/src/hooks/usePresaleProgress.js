...
const usePresaleProgress = () => {
  const [totalRaised, setTotalRaised] = useState(640);
  const [progressPercentage, setProgressPercentage] = useState(64.0);
  const [realTimeBalance, setRealTimeBalance] = useState(69120000);
  ...
  const fetchProgress = useCallback(async () => {
    try {
      ...
      if (cached) {
        const cachedTotal = Math.max(parseFloat(cached), 640);
